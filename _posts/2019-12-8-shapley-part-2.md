...
### Radical Shapley values vs. TreeExplainer
...
### Radical Shapley values vs. KernelExplainer
...
The two methods produce different but correlated results. Another way to summarize the differences is that if we sort and rank the Shapley values of each sample (from 1 to 6), the order would be different by about 0.75 ranks on average (e.g., in about 75% of the samples two adjacent features' order is switched). Let's remember that we are not looking at the relation between exact values and their noisy estimation: rather, **_radical Shapley values are a deterministic measure of one thing, and the kernel SHAP values are an estimation of another (related) thing_**.
